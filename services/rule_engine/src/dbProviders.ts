...
function toSsl(flag: string | undefined) {
  return String(flag).toLowerCase() === "true" ? { rejectUnauthorized: false } : false;
}

function makePgClient(connectionString: string | undefined, sslFlag: string | undefined): DbClient {
  if (!connectionString) {
    throw new Error("Missing database connection string.");
  }

  const pool = new Pool({ connectionString, ssl: toSsl(sslFlag) });
...
export async function makeDbClients() {
  const core = makePgClient(process.env.CORE_DATABASE_URL, process.env.CORE_PGSSL);
  const telemetry = makePgClient(process.env.TELEMETRY_DATABASE_URL, process.env.TELEMETRY_PGSSL);
  const security = makePgClient(process.env.SECURITY_DATABASE_URL, process.env.SECURITY_PGSSL);
...
export class DbRulesProvider implements RulesProvider {
  ...
  async loadActiveRules(): Promise<RuleRow[]> {
    ...
    return rows.map((r) => ({
      ...
      default_response_type: String(r.default_response_type || "MANUAL").toUpperCase() as RuleRow["default_response_type"],
      default_response_action: String(r.default_response_action || "Review alert"),
...
export class DbCapacityProvider implements CapacityProvider {
  ...
  async loadCapacityByZone(): Promise<Record<string, number>> {
    const { rows } = await this.coreDb.query<any>(`SELECT zone_id, zone_capacity FROM zones`);
