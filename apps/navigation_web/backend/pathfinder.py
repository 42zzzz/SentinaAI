...
Requirements:
- Use Dijkstra (no heuristic).
- Support dynamic edge weights updated by IoT data.

The graph is supplied as a list of nodes and edges in the format generated by navmesh_generator.py.
"""
...
class DijkstraPathfinder:
    ...
    def get_path_distance(self, path: List[str]) -> float:
        ...
        for i in range(len(path) - 1):
            a = path[i]
            b = path[i + 1]
            # Find weight from adjacency list
            w = None
