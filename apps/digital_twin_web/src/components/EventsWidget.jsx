...
export default function EventsWidget({ hallId }) {
  ...
  return (
    <div style={{ marginTop: '16px', borderTop: '1px solid var(--border-color)', paddingTop: '12px' }}>
      <h4 style={{ fontSize: '11px', color: 'var(--text-accent)', textTransform: 'uppercase', letterSpacing: '1px', margin: '0 0 10px 0' }}>
        Upcoming Events
      </h4>
      {sorted.map(evt => {
        ...
        return (
          <div key={evt.id} style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', marginBottom: '10px' }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ color: 'var(--text-primary)', fontWeight: 500, fontSize: '12px', lineHeight: 1.3 }}>{evt.title}</div>
