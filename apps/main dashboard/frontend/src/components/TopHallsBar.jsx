...
export default function TopHallsBar({ title = "Busiest halls (snapshot)", zoneId, eventId, limit = 8 }) {
      ...
      <div style={{ padding: 14, paddingTop: 0, display: "grid", gap: 10 }}>
        ...
        {!rows.length ? (
          ...
          rows.map((r) => {
            ...
            return (
              <div key={r.hall_id} style={{ display: "grid", gridTemplateColumns: "200px 1fr 70px", gap: 10, alignItems: "center" }}>
                ...
                <div style={{ height: 10, background: "#f1f5f9", borderRadius: 999, overflow: "hidden", border: "1px solid #e5e7eb" }}>
                  <div
                    style={{
                      width: `${widthPct}%`,
                      height: "100%",
                      background: overcrowded ? "#ef4444" : "#111827",
                    }}
                  />
                </div>
