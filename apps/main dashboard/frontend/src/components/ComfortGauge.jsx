...
// Assumption: comfort is roughly 0–100. If yours is different, adjust max.
export default function ComfortGauge({ title = "Comfort Index", value, subtitle }) {
  ...
  // thresholds
  const label =
    !has ? "" : v >= 75 ? "Excellent" : v >= 55 ? "Good" : v >= 40 ? "Moderate" : "Poor";
      ...
      <div style={{ marginTop: 50, paddingTop: 6, display: "grid", placeItems: "center" }}>
        ...
        <div style={{ width: 260, height: 140, position: "relative" }}>
          ...
          <div style={{ position: "absolute", left: 0, right: 0, top: 38, textAlign: "center" }}>
            ...
            <div style={{ marginTop: 6, fontSize: 13, opacity: 0.75, fontWeight: 800 }}>{label}</div>
          </div>
        </div>

        <div style={{ marginTop: 10, width: 260, display: "flex", justifyContent: "space-between", fontSize: 12, opacity: 0.7 }}>
          <span>0</span>
          <span>100</span>
        </div>
      </div>
