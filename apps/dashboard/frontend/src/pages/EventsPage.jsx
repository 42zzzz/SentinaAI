        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          ...
          <tbody>
            {loading ? (
              ...
              rows.map((r) => (
                <tr key={r.event_id} style={{ borderBottom: "1px solid #f1f5f9" }}>
                  <td style={tdStrong}>
                    <div style={{ fontWeight: 800 }}>{r.event_name}</div>
                    <div style={{ fontFamily: "monospace", fontSize: 12, opacity: 0.75 }}>{r.event_id}</div>
                  </td>
                  ...
                  <td style={td}>{Number(r.expected_attendance_total).toLocaleString()}</td>
                  <td style={td}>{Number(r.expected_exhibitors).toLocaleString()}</td>
                  <td style={td}>{Number(r.exhibitors_joined).toLocaleString()}</td>
                  <td style={td}>{formatAED(r.revenue_aed)}</td>
                  <td style={td}>
                    <div>{r.person_in_charge_name || "-"}</div>
                    <div style={{ fontSize: 12, opacity: 0.75 }}>{r.person_in_charge_email || ""}</div>
                  </td>
                  <td style={td}>
